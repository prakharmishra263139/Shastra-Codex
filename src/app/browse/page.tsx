              ...
              {FORCES.map((force) => (
                <li key={force.slug}>
                  <Link
                    ...
                    className="inline-flex items-center gap-2.5 rounded-[5px] border border-rule bg-surface px-3.5 py-2 text-[13px] text-ink-2 transition-colors hover:border-[var(--tone)] hover:text-ink"
                  >
          ...
          {TAXONOMY.map((category, i) => {
            ...
            return (
              <Link
                key={category.slug}
                href={`/${category.slug}`}
                className="group flex min-h-[13rem] flex-col bg-surface p-6 transition-colors hover:bg-surface-2"
              >
