...
import { Wrap, Eyebrow, ServiceRule } from "@/components/layout/section";
import {
  PageTransition,
  SharedPhoto,
  forcePhotoName,
  ...
  return (
    <PageTransition>
      <Wrap wide className="py-12 sm:py-16">
        ...
        <header className="mt-8 max-w-[46ch] pb-10">
        ...
        {/* Three services, three colours: the rule under the page title is the
            legend for the three cards below it. */}
        <ServiceRule />
        ...
        <section className="mt-10 grid gap-5 lg:grid-cols-3">
          {FORCES.map((force, i) => (
            <Link
              ...
              className="group reveal relative flex flex-col overflow-hidden rounded-xl border border-rule bg-surface shadow-[var(--shadow-card)] transition-[translate,border-color,box-shadow] duration-300 ease-soft hover:-translate-y-1 hover:border-tone hover:shadow-[var(--shadow-lift)]"
            >
              {/* The card's own service wash, under the content and over the
                  surface — the same field colour its page is built from. */}
              ...
              <div className="relative aspect-[16/10] overflow-hidden bg-surface-2">
                ...
                />
                <span aria-hidden className="tone-bg absolute inset-x-0 bottom-0 h-[3px]" />
              </div>

              <div className="relative flex flex-1 flex-col p-6">
                <Eyebrow toned>{force.unitLabel}</Eyebrow>

                <h2 className="mt-4 font-display text-[22px] font-bold leading-tight tracking-[-0.025em] transition-colors duration-200 ease-soft group-hover:text-tone">
                  {force.name}
                </h2>
                ...
                <p className="mt-4 flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  <span lang="sa" className="text-[15px] text-ink-2">
                    {force.motto.text}
                  </span>
